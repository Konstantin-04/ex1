...
var features = [
    {
        position: {lat: 47.232359, lng: 39.727854},
        center: {lat: 47.233465, lng: 39.714712},
        title: '<b>PROAUTO</b><br>г. Ростов-на-Дону, ул. Города Волос, 97, офис 16'
    }
];
...
function car_move() {
    var scrolled = window.pageYOffset || document.documentElement.scrollTop; // высота при прокрутке
        scrolled = scrolled;
        //var scrolled_l = window.pageXOffset || document.documentElement.scrollLeft;
        if(scrolled<=dh){
            $(".move-right").css({"-webkit-transform":"none","-moz-transform":"none","-o-transform":"none","-ms-transform":"none","transform":"none","left":"100","top":"0"});
        }
        ...
        if(scrolled>end_g && scrolled<=end_g1){
            ...
            i=(90*i)/100;
            $(".move-right").css({"-webkit-transform":"rotate("+i+"deg)","-moz-transform":"rotate("+i+"deg)","-o-transform":"rotate("+i+"deg)","-ms-transform":"rotate("+i+"deg)","transform":"rotate("+i+"deg)","left":g_dvij+100+"px","top":"0"});
        }
        if(scrolled>end_g1 && scrolled<=end_g1+300){
            $(".move-right").css({"-webkit-transform":"rotate(90deg)","-moz-transform":"rotate(90deg)","-o-transform":"rotate(90deg)","-ms-transform":"rotate(90deg)","transform":"rotate(90deg)","left":g_dvij+100+"px"});
        ...
        if(scrolled>end_g1+300 && scrolled<=end_g1+350){
            ...
            i=(g*100)/50; // сколько мы прошли
            i=90+(90*i)/100;
            $(".move-right").css({"-webkit-transform":"rotate("+i+"deg)","-moz-transform":"rotate("+i+"deg)","-o-transform":"rotate("+i+"deg)","-ms-transform":"rotate("+i+"deg)","transform":"rotate("+i+"deg)","left":g_dvij+100+"px","top":ddh+"px"});
        }
        if(scrolled>end_g1+350 && scrolled<=end_g1+900){
            g=scrolled-end_g1-350; //где мы находимся по y
            i=(g*200)/300; // сколько мы прошли
            i=(250*i)/100; 
            $(".move-right").css({"-webkit-transform":"rotate(-180deg)","-moz-transform":"rotate(-180deg)","-o-transform":"rotate(-180deg)","-ms-transform":"rotate(-180deg)","transform":"rotate(-180deg)","left":(g_dvij+100-i)+"px","top":ddh+"px"});
        }
}